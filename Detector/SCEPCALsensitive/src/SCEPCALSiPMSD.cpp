...
drc::SCEPCALSiPMSD::SCEPCALSiPMSD(const std::string aName,
...
drc::SCEPCALSiPMSD::~SCEPCALSiPMSD() {}



void drc::SCEPCALSiPMSD::Initialize(G4HCofThisEvent* hce) {

    fHitCollection = new drc::SiPMHitsCollection(SensitiveDetectorName,collectionName[0]);
    if (fHCID<0) { fHCID = GetCollectionID(0); }
    hce->AddHitsCollection(fHCID,fHitCollection);
}


G4bool drc::SCEPCALSiPMSD::ProcessHits(G4Step* step, G4TouchableHistory*) {
...
*/
    ...
    if (hit==NULL) {
        hit = new SiPMHit(fWavlenStep,fTimeStep);
        hit->SetSiPMnum(cID);
...
float drc::SCEPCALSiPMSD::findWavCenter(G4double en) {
    int i = 0;
    for ( ; i < fWavBin+1; i++) {
...
float drc::SCEPCALSiPMSD::findTimeCenter(G4double stepTime) {
    int i = 0;
    for ( ; i < fTimeBin+1; i++) {
